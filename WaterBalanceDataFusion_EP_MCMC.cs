...
namespace WaterBalanceDataFusion
{
    ...
    public partial class WaterBalanceDataFusion_EP : IGeneratedAlgorithm
    {
        ...
        /// </summary>
        /// <param name="modelFile">Where to save the results.</param>
        public void SaveAs(string modelFile)
        {
            string path = Path.GetDirectoryName(modelFile);
            if (!string.IsNullOrEmpty(path)) Directory.CreateDirectory(path);
            var serializer = new System.Runtime.Serialization.Formatters.Binary.BinaryFormatter();
            using (var stream = new FileStream(modelFile, FileMode.Create))
            {
        ...
        public double InferMCMC()
        {
            ...
            MCMC.DEMC_Z(GetLogProb, nIter, samples, logProbs);
            EstimateLatentMarginals(samples);
            int mode = logProbs.IndexOfMax();
    ...
    public static class MCMC
    {
        ...
        public static void DEMC_Z(Func<Vector, double> f, int N, List<Vector> X, List<double> F, double probGamma1 = 0.2)
        {
            int d = X[0].Count;
        ...
        /// <returns>Index of the (first occurrence) of the maximum value.</returns>
        public static int IndexOfMax(this IList<double> values)
        {

...
namespace WaterBalanceDataFusion
{
    class Program
    {
        ...
        private static void InferWaterBalance(string basinFolder)
        {
            ...
            // Write results
            wb.SaveAs($"{basinFolder}results.bin");
        ...
        private static void LoadData(string basinFolder, out double[][] P, out double[] PStd, out double[][] E, out double[] Q, out double[][] S)
        {
            ...
            using (var file = new StreamReader($"{basinFolder}fileNames.txt"))
            {
                char[] delim = { '\t', ' ', ',' };
            ...
            // Read data files
            Q = File.ReadAllLines($"{basinFolder}{fileNames["QObs"]}").Select(s => double.Parse(s)).ToArray();//mm/month
            P = new double[2][];
            P[0] = File.ReadAllLines($"{basinFolder}{fileNames["PObs1"]}").Select(s => double.Parse(s)).ToArray();//mm/month
            P[1] = File.ReadAllLines($"{basinFolder}{fileNames["PObs2"]}").Select(s => double.Parse(s)).ToArray();//mm/month
            PStd = File.ReadAllLines($"{basinFolder}{fileNames["PStd"]}").Select(s => double.Parse(s)).ToArray();//mm/month
            E = new double[2][];
            E[0] = File.ReadAllLines($"{basinFolder}{fileNames["EObs1"]}").Select(s => double.Parse(s)).ToArray();//mm/month
            E[1] = File.ReadAllLines($"{basinFolder}{fileNames["EObs2"]}").Select(s => double.Parse(s)).ToArray();//mm/month
            S = new double[1][];
            S[0] = File.ReadAllLines($"{basinFolder}{fileNames["SObs"]}").Select(s => double.Parse(s)).ToArray();//mm
